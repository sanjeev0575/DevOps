pipeline {
    ...
    environment {
        ...
        TASK_FAMILY          = 'python-app-task-automated-${BUILD_NUMBER}'
...
//                 withCredentials([aws(credentialsId: 'aws-cred', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
//                     script {
//                         sh 'envsubst < task-definition-template.json > task-definition-rendered.json'
...
//                             returnStdout: true
//                         ).trim()
//                         echo "Registered Task Definition ARN: ${env.TASK_DEFINITION_ARN}"
//                     }
//                 }
//             }
//         }
...
//                     script {
//                         def firstSubnet = env.SUBNET_IDS.split(',')[0]
//                         env.VPC_ID = sh(script: """
//                             aws ec2 describe-subnets --subnet-ids ${firstSubnet} --region ${AWS_REGION} --query 'Subnets[0].VpcId' --output text
...
//     post {
//         always {
//             sh 'docker system prune -f -a --volumes -f'
//         }
//         success {
//             echo '✅ Deployment pipeline completed successfully!'
//         }
